...
(function () {
    ...
    function clickListener(event) {
        ...
        try {
            ...
            try {
                if (this && this.dataset)
                    delete this.dataset.nnwProcessing;
            } catch (_) {}
            logger.groupEnd();
        }
    }
    ...
    function autoStartFileLink() {
        if (!/file_id=/.test(window.location.href))
            return;
        debugLog("autoStartFileLink detected file_id in URL - trying to auto-start");
        const findInRoot = (root) => {
            ...
            try {
                ...
                const dataEl = root.querySelector && root.querySelector("[data-download-url]");
                if (dataEl)
                    return dataEl;

                const candidates = root.querySelectorAll ? Array.from(root.querySelectorAll("button, a")) : [];
                for (const el of candidates) {
                    const text = (el.textContent || "").trim().replace(/\s+/g, " ");
                    if (/\bSlow download\b/i.test(text))
                        return el;
                }
            } catch (e) {
        ...
        const tryFind = () => {
            ...
            if (!button) {
                ...
                if (mf && mf.shadowRoot)
                    button = findInRoot(mf.shadowRoot);
            }
            return button;
        };
        ...
        const poll = setInterval(() => {
            ...
            if (!btn) {
                const mf = document.querySelector("mod-file-download");
                if (mf && mf.shadowRoot)
                    btn = findInRoot(mf.shadowRoot);
            }
            if (btn) {
                clearInterval(poll);
                debugLog("autoStartFileLink: found button after polling (attempts=" + attempts + ")", btn);
                try {
                    clickListener.call(btn, null);
                    closeOnDL();
                } catch (e) {
    ...
    function scrollToMainFiles() {
        try {
            if (!/\btab=files\b/.test(window.location.href))
                return;
